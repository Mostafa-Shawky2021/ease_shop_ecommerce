...
const CategoriesMenuMobile = () => {
	...
	return (
		<div className={style.categoriesMenuMobileWrapper}>
			<Button className={style.buttonCategories} onClick={handleCategoryList}>
				<BarIcon
				...
				<span
					...
					style={{
						transition: "all 0.3s",
						transform: categoryListIsOpen ? "rotate(-90deg)" : "rotate(0deg)",
					}}
				>
			...
			<Collapse in={categoryListIsOpen}>
				<ul className={`${style.listCategories} list-unstyled`}>
					{!!categories?.length ? (
						<ListItem
							data={mainCategories}
							renderItem={(category) => <CategoryItem key={category.id} categoryData={category} />}
						/>
					) : (
						<li>لا توجد اقسام</li>
					)}
